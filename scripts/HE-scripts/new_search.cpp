...
// Modern SHA-256 using EVP API (not used in this search code, but here for consistency)
...
int main() {
    ...
    if (!pub_in || !sec_in) {
        cerr << "Error: could not open key files." << endl;
        return 1;
    }
    ...
    if (!in.is_open()) {
        cerr << "Error: Could not open JSON data file." << endl;
        return 1;
    }
    json data;
    in >> data;
    ...
    if (!csv.is_open()) {
        cerr << "Error: Could not open email CSV file." << endl;
        return 1;
    }
    string header, row;
    getline(csv, header);
